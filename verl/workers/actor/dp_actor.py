        ...
        with torch.no_grad():
            if is_fsdp and getattr(self, 'redo_enabled', False):
                ...
                if self.global_steps % self.redo_metric_freq == 0:
                    ...
                    zero_grad_stats = analyze_all_fsdp_zero_grad_space(self.actor_module, tau=self.redo_tau, verbose=(rank==0), original_shapes_map=self.original_param_shapes, skip_mlp=True, skip_embed=True, current_step=self.global_steps) # <<< Cascade: Pass original_param_shapes and skip MLP/embedding layers
