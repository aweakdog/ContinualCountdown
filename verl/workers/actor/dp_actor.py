        ...
        try:
            ...
            try:
                ...
                if first_worker:
                    ...
                    print(f"[CK_RESET_DEBUG] Actor: Available methods on ref_worker: {available_methods[:20]}...")  # Show first 20
                    
                    # Check specifically for get_layer_parameters
                    has_method = hasattr(first_worker, 'get_layer_parameters')
                    print(f"[CK_RESET_DEBUG] Actor: ref_worker has get_layer_parameters method: {has_method}")
                    
                    # Check worker type
                    worker_type = type(first_worker).__name__
                    print(f"[CK_RESET_DEBUG] Actor: ref_worker type: {worker_type}")
                else:
