...
export CHECKPOINT_BASE_DIR=${CHECKPOINT_BASE_DIR:-/nas/shared/sys2/yuanhangli/tmp/checkpoints/continual_countdown3b_grpo}
...
export GRPO_N_SAMPLES=${GRPO_N_SAMPLES:-5}  # Number of samples per prompt for GRPO
export GRPO_KL_COEF=${GRPO_KL_COEF:-0.001}  # KL loss coefficient for GRPO
...
echo "[GRPO Config] Using $GRPO_N_SAMPLES samples per prompt, KL coefficient: $GRPO_KL_COEF"

# Set up logging with backup
LOG_FILE="./grpo_logs/ContinualCountdown3B_GRPO_SingleRun.log"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
BACKUP_DIR="./grpo_logs/run"
...
rm -rf ./checkpoints/continual_countdown3b_grpo
rm -rf ${CHECKPOINT_BASE_DIR}

# Create all required directories first
mkdir -p ./grpo_logs
mkdir -p ./grpo_logs/run
mkdir -p ./checkpoints/continual_countdown3b_grpo
...
WANDB_RUN_NAME="ContinualCountdown3B_GRPO_SingleRun"
log_file="./grpo_logs/${WANDB_RUN_NAME}.log"

# Print debug info
echo "Starting ContinualCountdown3B GRPO training at $(date)" | tee -a "$log_file"
echo "Current directory: $(pwd)" | tee -a "$log_file"
echo "Python path: $(which python3)" | tee -a "$log_file"

echo "GRPO Training configuration:" | tee -a "$log_file"
...
echo "  GRPO N_SAMPLES: $GRPO_N_SAMPLES" | tee -a "$log_file"
echo "  GRPO KL_COEF: $GRPO_KL_COEF" | tee -a "$log_file"
echo "  WANDB_MODE: $WANDB_MODE" | tee -a "$log_file"
echo "  CHECKPOINT_BASE_DIR: $CHECKPOINT_BASE_DIR" | tee -a "$log_file"

# Set up experiment logging
MASTER_LOG_FILE="./grpo_logs/ContinualCountdown3B_GRPO_MasterLog.log"
EXP_LOG_DIR="./grpo_logs/experiments"
mkdir -p "$EXP_LOG_DIR"

echo "=== Starting ContinualCountdown3B GRPO Curriculum Training ===" | tee -a "$MASTER_LOG_FILE"
echo "Training started at: $(date)" | tee -a "$MASTER_LOG_FILE"
echo "Base model: $BASE_MODEL" | tee -a "$MASTER_LOG_FILE"
echo "Algorithm: GRPO (Group Relative Policy Optimization)" | tee -a "$MASTER_LOG_FILE"
...
for ((phase1_iter=1; phase1_iter<=PHASE1_REPEAT_COUNT; phase1_iter++)); do
  ...
  RUN_NAME="Phase1_Group${group}_Iter${phase1_iter}_GRPO_$(date +%Y%m%d_%H%M%S)"
  export RUN_NAME
  LOG_FILE="$EXP_LOG_DIR/${RUN_NAME}.log"
  echo "Training group $group with GRPO (iteration $phase1_iter)" | tee -a "$LOG_FILE" | tee -a "$MASTER_LOG_FILE"
...
python3 -m verl.trainer.main_ppo \
  ...
  actor_rollout_ref.actor.kl_loss_coef=$GRPO_KL_COEF \
  +actor_rollout_ref.actor.kl_coeff=$GRPO_KL_COEF \
  ...
  actor_rollout_ref.rollout.n=$GRPO_N_SAMPLES \
  ...
  algorithm.kl_ctrl.kl_coef=$GRPO_KL_COEF \
