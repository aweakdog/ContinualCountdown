#!/usr/bin/env python3
"""
Plot LLaMA Instruct DeepMath performance comparison across different global steps.
...
class LlamaInstructDeepMathPlotter:
    def __init__(self, base_dir="/cpfs04/user/liyuanhang.p/src/ContinualCountdown/logs/instruct/llama/continual", 
                 output_dir="./plots/llama_deepmath"):
        ...
        self.step_dirs = {
            0: "debug_continual_llama_sft_global_step_0_reset_k0f_k0l_20250824_065715",
            5: "develop_continual_llama_sft_global_step_5_reset_k0f_k0l_20250824_065814", 
            10: "repeat_continual_llama_sft_global_step_10_reset_k0f_k0l_20250824_065844",
            15: "temp_continual_llama_sft_global_step_15_reset_k0f_k0l_20250824_065916"
        }
    ...
    def parse_critic_score_data(self, global_step):
        """Parse critic score data from log files for a specific global step"""
        step_dir = self.step_dirs.get(global_step)
        if not step_dir:
            return [], []
            
        log_dir = self.base_dir / step_dir
        if not log_dir.exists():
            print(f"Directory not found: {log_dir}")
            return [], []
            
        # Look for all log files with pattern Exp2_Group1and2_Iter*_SFT_global_step_X_*.log
        log_files = list(log_dir.glob(f"Exp2_Group1and2_Iter*_SFT_global_step_{global_step}_*.log"))
        print(f"Global step {global_step}: Found {len(log_files)} log files", end="")
        
        # Collect data from all log files
        all_data = {}  # step -> list of scores
            ...
            try:
                with open(log_file, 'r', encoding='utf-8', errors='ignore') as f:
                    for line in f:
                        ...
                        if 'step' in metrics and 'fisher/C_K_normalized' in metrics:
                            step = metrics['step']
                            score = metrics['fisher/C_K_normalized']
                            if step not in all_data:
                                all_data[step] = []
                            all_data[step].append(score)
            except Exception as e:
        ...
        scores = [np.mean(all_data[step]) for step in steps]
        
        print(f", {len(steps)} data points (averaged across {len(log_files)} files)")
        return steps, scores
    ...
    def collect_data_for_global_step(self, global_step):
        ...
        log_pattern = f"Exp2_Group1and2_Iter*_SFT_global_step_{global_step}_*.log"
    ...
    def plot_critic_score_comparison(self, smooth=True, sigma=1.5):
        ...
        title = 'LLaMA Instruct DeepMath Performance: Critic Score Comparison'
        if smooth:
            title += ' (Smoothed)'
        ...
        plt.tight_layout()
        output_path = self.output_dir / "llama_critic_score_comparison.png"
    ...
    def plot_fisher_ck_comparison(self, smooth=True, sigma=1.5):
        ...
        title = 'LLaMA Instruct DeepMath Performance: Fisher C_K_normalized Comparison'
        if smooth:
            title += ' (Smoothed)'
        ax.set_title(title, fontsize=14, fontweight='bold')
        ax.legend(fontsize=10)
        ...
        plt.tight_layout()
        output_path = self.output_dir / "llama_fisher_ck_comparison.png"
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_fisher_ck_lowercase_comparison(self, smooth=True, sigma=1.5, remove_outliers=True):
        ...
        title = 'LLaMA Instruct DeepMath Performance: Fisher c_k Normalized (lowercase) Comparison'
        #if remove_outliers:
        ...
        plt.tight_layout()
        output_path = self.output_dir / "llama_fisher_ck_lowercase_comparison.png"
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_computed_ck_comparison(self, smooth=True, sigma=1.5, remove_outliers=True):
        ...
        title = 'LLaMA Instruct DeepMath Performance: Computed C_K from c_k Averages'
        #if remove_outliers:
            #title += ' (Outliers Removed)'
        if smooth:
            title += ' (Smoothed)'
        ...
        plt.tight_layout()
        output_path = self.output_dir / "llama_computed_ck_comparison.png"
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_sliding_window_ck_comparison(self, smooth=True, sigma=1.5, remove_outliers=True, window_size=20, task_boundary=150):
        ...
        plt.tight_layout()
        output_path = self.output_dir / f"llama_sliding_window_ck_comparison_w{window_size}.png"
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_raw_sliding_window_ck_comparison(self, window_size=20, task_boundary=150):
        ...
        plt.tight_layout()
        output_path = self.output_dir / f"llama_raw_sliding_window_ck_comparison_w{window_size}.png"
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_log_sliding_window_ck_comparison(self, window_size=20, task_boundary=150):
        ...
        plt.tight_layout()
        output_path = self.output_dir / f"llama_log_sliding_window_ck_comparison_w{window_size}.png"
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_control_group_critic_score(self, smooth=True, sigma=1.5):
        ...
        ax.set_xlabel('Training Step', fontsize=12)
        ax.set_ylabel('Critic Score Mean', fontsize=12)
        title = 'LLaMA Instruct DeepMath Control Group: Critic Score Performance'
        if smooth:
            title += ' (Smoothed)'
        ax.set_title(title, fontsize=14, fontweight='bold')
        ax.legend(fontsize=10)
        ...
        plt.tight_layout()
        output_path = self.output_dir / "llama_control_group_critic_score.png"
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_summary_statistics(self):
        ...
        plt.tight_layout()
        output_path = self.output_dir / "llama_summary_statistics.png"
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.show()
...
def main():
    ...
    plotter = LlamaInstructDeepMathPlotter()
    
    print("Generating LLaMA Instruct DeepMath performance comparison plots...")
    print("=" * 60)
