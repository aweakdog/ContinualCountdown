...
# Enhanced SFT Training Script for Qwen 3B Model
# Usage: ./train_continual_countdown_3b_sft_qwen.sh [SFT_SIZE] [--gpus NUM_GPUS] [--model MODEL_PATH] [--wandb MODE]
# Example: ./train_continual_countdown_3b_sft_qwen.sh 2048 --gpus 8 --wandb online
...
show_usage() {
    ...
    echo "  --model        Path to base model (default: /cpfs04/user/liyuanhang.p/model/qwen_instruct3b)"
...
SFT_SIZE=${1:-2048}
NUM_GPUS=8
BASE_MODEL="/cpfs04/user/liyuanhang.p/model/qwen_instruct3b"
WANDB_MODE="disabled"
...
echo "=== Qwen 3B SFT Training Configuration ==="
...
LOG_DIR="./logs/sft_qwen"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/sft_qwen_${SFT_SIZE}_${TIMESTAMP}.log"

echo "Starting Qwen 3B SFT training..." | tee "$LOG_FILE"
...
CONFIG_PATH="/cpfs04/user/liyuanhang.p/src/ContinualCountdown/verl/trainer/config/sft_qwen_instruct_trainer.yaml"
...
torchrun --standalone --nnodes=1 --nproc_per_node=$NUM_GPUS --master_port=29500 \
    ...
    --config-name sft_qwen_instruct_trainer \
    2>&1 | tee -a "$LOG_FILE"
...
if [ $TRAINING_EXIT_CODE -eq 0 ]; then
    ...
    echo "Model: Qwen 3B" | tee -a "$LOG_FILE"
