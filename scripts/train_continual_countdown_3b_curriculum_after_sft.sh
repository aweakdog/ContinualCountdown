...
# Two-phase training: Phase 1 (group0 only), Phase 2 (group1 + group2 in same session)

# Phase 1: Train only group0
echo "=== PHASE 1: Training Group 0 ==="
group=0
...
# Phase 2: Train group1 and group2 in the same training session
echo "=== PHASE 2: Training Group 1 + Group 2 ==="
...
RUN_NAME="Phase2_Group1and2_$(date +%Y%m%d_%H%M%S)"
LOG_FILE="$EXP_LOG_DIR/${RUN_NAME}.log"
echo "Training groups 1 and 2 sequentially" | tee -a "$LOG_FILE" | tee -a "$MASTER_LOG_FILE"
echo "Train files: $TRAIN_FILES_STR" | tee -a "$LOG_FILE" | tee -a "$MASTER_LOG_FILE"
echo "Val files: $VAL_FILES_STR" | tee -a "$LOG_FILE" | tee -a "$MASTER_LOG_FILE"
